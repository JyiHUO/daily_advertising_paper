...
### 2. Let the Agent Steer: Closed-Loop Ranking Optimization via Influence Exchange
- 挑选理由：闭环排序优化可能与广告排序有同构性，但缺乏摘要无法确认具体场景，标题中ranking optimization有潜在迁移价值

### 3. Hidden Ads: Behavior Triggered Semantic Backdoors for Advertisement Injection in Vision Language Models
...
*图示：该图中左上包含论文的核心方法总览图（Figure 1），能看到编码器/解码器、Reverse curriculum 相关模块以及信息流，属于方法框架示意；相比其他候选，page-3-block-35 是实验曲线和表格，page-0 的几个区域基本没有主图内容，page-1-region-upper-right 主要是正文公式。虽然该候选裁剪中还带有部分正文，但仍是最能代表论文核心方法的一张。*
...
*图示：这张图说明 RCLRec 如何先从长历史里反向筛出与购买最相关的几步，再把它们作为课程前缀来强化稀疏转化学习。*
...
*图示：这张图说明：RCLRec怎样在前向时硬选Top-k历史、在反向时保留可学习通道，让“课程前缀”能被最终转化目标直接优化。*
...
*图示：这张图说明质量感知损失如何把“挑前缀”变成“只有真正提升购买预测的前缀才会被保留”。*
...
*图示：这张图用证据链说明：RCLRec的收益来自挑出少量高意图前缀并约束其质量，而不是简单延长用户历史。*
...
*图示：这张图最直接展示了论文核心方法与行为差异：上中下三部分分别对比传统后门、正常使用和作者提出的 Hidden Ads，清楚呈现了触发条件、VLM 响应变化以及“语义+意图”双重触发后插入广告语的机制。它不是实验结果图，而是方法/攻击行为总览图，最能代表论文主旨。相比页面级裁剪，这个内嵌图更完整、聚焦、可读性更好。*
...
*图示：把“行为触发后门”的技术定义、直觉理解和例子串成一条理解路径。*
...
*图示：这张图把论文中的三种攻击深度串成一条路径，帮助读者看懂攻击者从改提示到改模型本体的递进方式。*
...
*图示：这张图说明论文如何通过成组的带毒与反例样本，把广告注入训练成一种只有“目标语义+推荐意图”同时出现才触发的隐蔽行为。*
...
*图示：这张图用四步说明：论文如何证明广告注入后门能在触发时稳定插入推广内容，同时在大多数正常任务中保持表面性能不变。*
